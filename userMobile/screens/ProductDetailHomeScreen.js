...
const ProductDetailHomeScreen = ({ route }) => {
  ...
  const handleAddToCart = async () => {
    try {
      const response = await axios.post('http://192.168.195.23:8000/api/cart', {
        user_id: userId,
      ...
      >
        <View style={styles.modalContainer}>
          <View style={styles.modalContent}>
            ...
            >
              {product.variations.map((variation) => (
                <Picker.Item key={variation.id} label={variation.name} value={variation} />
              ))}
            </Picker>
            ...
            {selectedVariation && (
              <>
                <Text style={styles.unitPrice}>Unit Price: Rp{formatPrice(selectedVariation.price)}</Text>
                <Text style={styles.totalPrice}>Total Price: Rp{formatPrice(selectedVariation.price * quantity)}</Text>
              </>
            )}
