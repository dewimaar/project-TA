...
const ProductDetailHomeScreen = ({ route }) => {
    ...
    const fetchProductDetails = async () => {
      try {
        const response = await axios.get(
          `http://192.168.99.23:8000/api/products/detail/${productId}`
        );
  ...
  useEffect(() => {
    const fetchUserId = async () => {
      try {
        ...
        const response = await axios.get(
          "http://192.168.99.23:8000/api/user",
          {
            headers: {
  ...
  const fetchStore = async () => {
    try {
      const response = await fetch(
        `http://192.168.99.23:8000/api/stores/${userId}`
      );
      if (response.ok) {
  ...
  const handleAddToCart = async () => {
    try {
      const response = await axios.post("http://192.168.99.23:8000/api/cart", {
        user_id: userId,
        store_id: storeId,
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      <Image
        ...
        source={{ uri: `http://192.168.99.23:8000/storage/${product.image}` }}
        resizeMode="contain"
      />
        ...
        <View key={variation.id} style={styles.variationContainer}>
          ...
          {variation.image && (
            <Image
              ...
              source={{
                uri: `http://192.168.99.23:8000/storage/${variation.image}`,
              }}
              resizeMode="contain"
            />
          )}
